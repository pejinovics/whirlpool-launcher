...
func main() {
	...
	for {
		...
		if !alive {
			// Ako nije alive, čekaj 1s i proveri ponovo
			time.Sleep(1 * time.Second)
			continue
		}

		// Pokreni listener samo kada je alive
		...
		// Accept loop - radi dok je alive
		for {
			// Postavi deadline da ne blokira zauvek
			...
			// Proveri da li je još uvek alive PRE procesiranja konekcije
			elapsed := time.Since(startTime)
			if !isAlive(elapsed) {
				log.Printf("[TCP] 🔴 LISTENER STOPPING (%v elapsed)", elapsed.Round(time.Second))
				listener.Close()
				break // Izađi iz accept loop-a
			}

			if err != nil {
				if netErr, ok := err.(net.Error); ok && netErr.Timeout() {
					continue // Timeout je OK
				}
				break // Pravi error, izađi
			}
